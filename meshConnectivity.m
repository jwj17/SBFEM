...
if ~iscell(sdConn) %ltx \texttt{sdConn} is a matrix (triangular elements) 
    ...
    sdEdge = cell(nsd,1); %ltx initialization
    i1 = 1; %ltx counter of edges
    meshEdge = cell(nsd,1); %ltx initialization
...
asd = zeros(length(a),1); %ltx initialization
...
edge2sd = cell(nMeshedges,1); %ltx initilization 
...
node2Edge = cell(nNode,1); %ltx initilization
...
node2sd = cell(nNode,1); %ltx initilization
