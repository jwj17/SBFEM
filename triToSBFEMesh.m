...
%ltx triangular element numbers will be the nodal numbers of S-elements.
...
for ii = 1:np
    if bpIdx(ii) == 0 %ltx interior point \label{triToSBFEMeshInterior}
        ...
        %ltx maintain counterclock direction and rearrange the nodes 
        %ltx as:[boundary node 1, current point (node), 
        %ltx boundary node 2, others]
        if abs(idx1-idx2) > 1
