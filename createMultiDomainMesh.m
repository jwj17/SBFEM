function [coord,sdConn,sdSC,sdMat] = createMultiDomainMesh(dDefs, para)
%cbgnltx 
%\myCodeTitle{% 
...
for id = 1:nDomain %ltx loop over domain \label{createMultiDomainMeshDomain}
    [ coord, sdConn, sdSC, sdMat ] = createSBFEMesh( ...
    ...
    d{id} = struct('coord',coord, 'sdConn',{sdConn}, 'sdSC',sdSC, ...
         'meshEdge',{meshEdge}, 'sdEdge',{sdEdge},'sdMat',sdMat);
...
for id = 2:nDomain %ltx loop over the remaining domains
    [ da.coord, da.sdConn, da.sdSC, da.sdMat] = combineTwoMeshes( ...
        {d{id}, da}, dDefs.onLine{id-1}, res);
    [ da.meshEdge, da.sdEdge] = meshConnectivity( da.sdConn );
end

coord = da.coord; sdConn = da.sdConn; sdSC = da.sdSC; sdMat = da.sdMat; %ltx output
